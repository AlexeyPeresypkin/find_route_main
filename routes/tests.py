...
class AllTestsCase(TestCase):
    ...
    def test_model_city_dublicate(self):
        '''Try to create dublicate city'''
        city = City(name='A')
        with self.assertRaises(ValidationError):
            city.full_clean()

    def test_model_train_dublicate(self):
        '''Try to create dublicate train'''
    ...
    def test_model_train_train_dublicate(self):
        ...
        try:
            ...
            self.assertEqual({'__all__': ['Необходимо изменить время в пути']},
                             e.message_dict)
    ...
    def test_message_error_from_cities(self):
        data = {'from_city': self.city_B.id, 'to_city': self.city_E.id,
                ...
                'travelling_time': 999}
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Маршрут, через эти города невозможен',
                            1, 200)
